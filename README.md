...
**0.2.0 覆盖设置面板、常用导航及原生菜单，仍不是完整中文语言包。** 词库包含 331 条精确词条。仅接受 Antigravity **2.12.2** 且 `dist/preload.js` 和 `dist/menu.js` SHA-256 与已检查样本一致的客户端。未知版本、其他汉化工具修改过的入口都拒绝安装；版本号和哈希匹配不代表验证了整个安装包的官方来源。
